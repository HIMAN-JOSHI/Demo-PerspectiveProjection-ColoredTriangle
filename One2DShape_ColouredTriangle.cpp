// This program is a demo for perspective projection using a coloured 2D object (triangle
...
Display *gpDisplay = NULL; // 'Display' is a connection (technically a struct) which is used by the X client to communicate with the X server (O.S)
XVisualInfo *gpXVisualInfo = NULL; // 'XVisualInfo' is a struct that stores information about the visual (More details mentioned in notes in createWindow() where this variable is used).
...
int main(void){
	...
	uninitialize();
	return 0; // Program never reaches until this statement. Program exits from case 33.
}
...
void display(void){
	...
	// Step 2: Perform the actual drawing
	glLoadIdentity();
	glTranslatef(0.0f, 0.0f, -3.0f);
	glBegin(GL_TRIANGLES);
		// Note:- The colors get interpolated.
		glColor3f(1.0f,0.0f,0.0f); // red: Color of apex of the triangle
		...
		glColor3f(0.0f, 0.0f, 1.0f); // blue: Color of right-bottom of the triangle
...
void toggleFullscreen(void){
	...
	wm_state = XInternAtom(gpDisplay, "_NET_WM_STATE", False); // XInternAtom will return the atom associated with the "_NET_WM_STATE" message where '_NET' stands for the Network Compliant and 'WM' for Window Message. False - Don't create a seperate atom, if it already exists.

	// Step 2: Create a custom event (or client event)
	...
	// Send the custom event to the event queue
	XSendEvent(gpDisplay, // Connection with the X-server
		   RootWindow(gpDisplay,gpXVisualInfo->screen), // Propogate the message/event to this window
...
void resize(int width, int height){
	...
	glViewport(0,0,(GLsizei) width, (GLsizei) height); // 1st param - x, 2nd param - y
...
void uninitialize(void){ // works like a destructor i.e. destroy in the reverse order of creation
	...
	if(gWindow){
		glXDestroyWindow(gpDisplay, gWindow);
	}
...
void createWindow(void){// Note:- All the programs which create their own window are X-clients of the X-server (i.e. O.S)
	...
	static int frameBufferList[] = {
		...
		GLX_RED_SIZE, 1, // The returned visual should support a buffer (1-bit atleast) of the red color.
		GLX_GREEN_SIZE,1, // The returned visual should support a buffer (1-bit atleast) of the green color.
		GLX_BLUE_SIZE, 1, // The returned visual should support a buffer (1-bit atleast) of the blue color.
		GLX_ALPHA_SIZE,1, // The returned visual should support a buffer (1-bit atleast) of the alpha.
	...
	if(gpDisplay==NULL){
		...
		uninitialize();
		exit(1); // Abortive exit and hence a positive number is used.
	}
	...
	// Note:- A single display(struct) can support multiple screens. Each screen can have several different visual types supported at different depths.
	...
	XStoreName(gpDisplay, gWindow, "Demo - 2D Colored triangle using perspective projection");

	// Step 7: Process the window 'close' event
	
	// Step 7.1: Create an atom to handle the close event
	// Note:- 'Atom' is an unique string and it stays in the memory until the app ends
	Atom windowManagerDelete = XInternAtom(gpDisplay, "WM_DELETE_WINDOW", True); // XInternAtom returns the atom id associated with "WM_DELETE_WINDOW" event/message (WM stands for Window Message), True - create an atom even if it exists.
